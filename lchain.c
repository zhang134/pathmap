...
uint64_t *mg_chain_backtrack(void *km, int64_t n, const int32_t *f, const int64_t *p, int32_t *v, int32_t *t, int32_t min_cnt, int32_t min_sc, int32_t max_drop, int32_t *n_u_, int32_t *n_v_)
{
	...
	uint64_t *u;
	int64_t i, k, n_z, n_v;
	int32_t n_u, my_score_threshold = 8;

	*n_u_ = *n_v_ = 0;
	for (i = 0, n_z = 0; i < n; ++i) // precompute n_z
		if (f[i] >= my_score_threshold) ++n_z;
		//////if (f[i] >= min_sc) ++n_z;
	if (n_z == 0) return 0;
	z = Kmalloc(km, mm128_t, n_z);
	for (i = 0, k = 0; i < n; ++i) // populate z[]
		if (f[i] >= my_score_threshold) z[k].x = f[i], z[k++].y = i;  // this 22 score including the original 15(kmer size)
	...
	for (k = n_z - 1, n_v = n_u = 0; k >= 0; --k) { // precompute n_u
		if (t[z[k].y] == 0) {
			int64_t n_v0 = n_v, end_i;     // n_v recores the anchors number in the chain ending at z[k].y
			int32_t sc;
			end_i = mg_chain_bk_end(max_drop, z, f, p, t, k);
			for (i = z[k].y; i != end_i; i = p[i])
				++n_v, t[i] = 1;           // t[]=1 indicates that it is in the chain, or is is checked in the backtrace
			sc = i < 0? z[k].x : (int32_t)z[k].x - f[i];
			//////if (sc >= min_sc && n_v > n_v0 && n_v - n_v0 >= min_cnt)
			if (sc >= my_score_threshold && n_v > n_v0 && n_v - n_v0 >= min_cnt)
				++n_u;
			else n_v = n_v0;
		}
	}
	...
	for (k = n_z - 1, n_v = n_u = 0; k >= 0; --k) { // populate u[]
		if (t[z[k].y] == 0) {
			int64_t n_v0 = n_v, end_i;   // n_v recores the anchors number in the chain, all chains, not only one chain
			int32_t sc;
			end_i = mg_chain_bk_end(max_drop, z, f, p, t, k);
			for (i = z[k].y; i != end_i; i = p[i])
				v[n_v++] = i, t[i] = 1;
			sc = i < 0? z[k].x : (int32_t)z[k].x - f[i];
			//////if (sc >= min_sc && n_v > n_v0 && n_v - n_v0 >= min_cnt)
			if (sc >= my_score_threshold && n_v > n_v0 && n_v - n_v0 >= min_cnt) // this means the new chain has at least min_cnt anchors
				u[n_u++] = (uint64_t)sc << 32 | (n_v - n_v0);
			else n_v = n_v0;             // n_v0 is the anchors number in the previous all checked chains
		}
	}
	kfree(km, z);
...
{
	// here is my scoring scheme, just adding 1 if satifying the original conditions in this code
	// just adding 1 can clearly know how many anchors contained
	int32_t dq = (int32_t)ai->y - (int32_t)aj->y, dr, dd;
	if (dq <= 0 || dq > max_dist_x) return INT32_MIN;
	dr = (int32_t)(ai->x - aj->x);
	if (dr == 0 || dq > max_dist_y) return INT32_MIN;
	dd = dr > dq? dr - dq : dq - dr;
	if (dd > bw) return INT32_MIN;
	if (n_seg > 1 && !is_cdna && dr > max_dist_y) return INT32_MIN;
	return 1;
/*
...
{ // TODO: make sure this works when n has more than 32 bits
	...
	p = Kmalloc(km, int64_t, n); // store the kmer where max score comes from, -1 means does not have the previous anchor
	f = Kmalloc(km, int32_t, n); // store the score of the i-th anchor
	v = Kmalloc(km, int32_t, n); // v[] keeps the peak score up to i, these must be in the chain, not solo anchor; 
	t = Kcalloc(km, int32_t, n); // is its ended anchor that starting from current anchor in the skeleton, other wise 0

	// fill the score and backtrack arrays
	for (i = 0, max_ii = -1; i < n; ++i) {
		int64_t max_j = -1, end_j;
		////// int32_t max_f = a[i].y>>32&0xff, n_skip = 0;
		int32_t max_f = 1, n_skip = 0; 
					   ...
					   int64_t n, mm128_t *a, int *n_u_, uint64_t **_u, void *km)
{
	...
	p = Kmalloc(km, int64_t, n); // store the kmer where max score comes from
	...
	for (i = i0 = 0; i < n; ++i) {
		...
		// set max
		//if(max_j < 0 || (a[max_j].x < a[i].x && (int32_t)a[max_j].y < (int32_t)a[i].y)){
		//	int haha = 1;
		//}
