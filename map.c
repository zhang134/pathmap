...
void mm_map_frag(const mm_idx_t *mi, int n_segs, const int *qlens, const char **seqs, int *n_regs, mm_reg1_t **regs, mm_tbuf_t *b, const mm_mapopt_t *opt, const char *qname)
{
	...
	//fprintf(stderr, "Locate %10d -th sequence done\r", 0);
    //fflush(stderr);
	...
	if (opt->flag & MM_F_RMQ) {
		...
		////// the u is the final chainning, n_regs0 is the number of chains, the output a has been changed, the first top is the anchors in selected chains
		/////  u[i]>>32 records the chaining score, (int32_t)u[i] records the anchors number in the chain	
	}
...
static void *worker_pipeline(void *shared, int step, void *in)
{
    ...
    if (step == 0) { // step 0: read sequences
		...
		if (mm_verbose >= 3){
			...
			time(&t);
			ctime_r(&t,buf);
			fprintf(stderr, "[INFO] average %d sequences mapped per secend.\n", s->n_seq / (int)(realtime() - mm_realtime0));
